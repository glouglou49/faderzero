...
function buildTransitionMeta(entry: SetlistSongDetail, durationSeconds: number) {
  const parts: string[] = [];
...
function buildPdfEntries(
  ...
  const pdfEntries: PdfEntry[] = entries.map((entry, index) => ({
    ...
    metaText: clampText(buildTransitionMeta(entry, songDurationsById.get(entry.songId) ?? 0), 76),
